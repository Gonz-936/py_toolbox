# src/py_toolbox/processing/tika_parser.py
import logging
from pathlib import Path
from tika import parser as tika_parser

class TikaParser:
    """
    Una clase de utilidad para interactuar con Apache Tika.
    """
    @staticmethod
    def pdf_to_html(pdf_path: str, output_dir: str) -> str | None:
        """
        Convierte un archivo PDF a HTML usando Tika y lo guarda en un directorio.

        :param pdf_path: La ruta al archivo PDF de entrada.
        :param output_dir: El directorio donde se guardará el archivo HTML.
        :return: La ruta al archivo HTML generado o None si falla.
        """
        pdf_path_obj = Path(pdf_path)
        output_dir_obj = Path(output_dir)
        output_dir_obj.mkdir(parents=True, exist_ok=True)

        html_file_name = pdf_path_obj.stem + ".html"
        html_out_path = output_dir_obj / html_file_name

        logging.info(f"Tika: Iniciando conversión de PDF a HTML para: {pdf_path}")
        try:
            parsed = tika_parser.from_file(str(pdf_path), xmlContent=True)
            html = parsed.get("content")

            if not html:
                logging.error("Tika no pudo extraer contenido del PDF.")
                return None

            html_out_path.write_text(html, encoding="utf-8")
            logging.info(f"Tika: HTML generado en: {html_out_path}")
            return str(html_out_path)
        except Exception as e:
            logging.error(f"Tika: Ocurrió un error procesando el PDF: {e}", exc_info=True)
            return None